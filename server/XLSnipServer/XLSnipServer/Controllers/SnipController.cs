...
namespace XLSnipServer.Controllers
{
    public class SnipController : Controller
    {
        ...
        [HttpPost]
        public JsonResult TargetedUploadRange(String UserName, String RangeName, String Description, String Address, String AllowedUsers, String Data)
        {
            using (var context = new xlsnippingtoolEntities())
            {
                ...
                if (UserRangeEntity == null)
                {
                    ...
                    range.RangeName = RangeName;
                    range.UserId = userEntity.Id;
                    range.AllowedUsers = AllowedUsers;
                    context.UserRanges.Add(range);
                    context.SaveChanges();
                ...
                else
                {
                    ...
                    UserRangeEntity.RangeName = RangeName;
                    UserRangeEntity.Description = Description;
                    UserRangeEntity.AllowedUsers = AllowedUsers;
                    context.SaveChanges();
        ...
        public ActionResult ListMyRanges(String UserName)
        {
            ...
            using (var context = new xlsnippingtoolEntities())
            {
                var qry = context.UserRanges.Where(r => r.AllowedUsers.Contains(UserName) || String.IsNullOrEmpty(r.AllowedUsers)).ToList();
                var usersList = context.Users.ToList();
                foreach (var item in qry)
                {
                    var qryUser = usersList.Where(x => x.Id.Equals(item.UserId));
